...
public class Builder : MonoBehaviour
{
    ...
    public void ToCreateNewBuilding(int i) //Start call from ActionManager UI Btns
    {
        ...
        if (!unit.Fraction.CheckBuildingCost(b)) //don't have enough resource to build
            return;
        else
        {
            ...
            ghostBuilding = Instantiate(ghostBuildingList[i],
                Input.mousePosition,
                Quaternion.identity, unit.Fraction.GhostBuildingParent);
    ...
    private void GhostBuildingFollowsMouse()
    {
        if (showGhost)
        {
            Ray ray = CameraController.instance.CamController.ScreenPointToRay(Input.mousePosition);
            RaycastHit hit;
    ...
    public void CreateBuildingSite(Vector3 pos) //Set a building site
    {
        ...
        buildingObj.transform.parent = unit.Fraction.BuildingsParent.transform;

        inProgressBuilding = buildingObj; //set a new clone building object to be a building in Unit's mind
        unit.Fraction.AliveBuildings.Add(building);

        building.Faction = unit.Fraction; //set a building's faction to be belong to this player
        building.IsFunctional = false;
        building.CurHP = 1;

        unit.Fraction.DeductBuildingCost(building);

        toBuild = false; //Disable flag at the builder
        showGhost = false; //Disable to show ghost building

        if (unit.Fraction == GameManager.instance.MyFaction)
        {
            MainUI.instance.UpdateAllResource(unit.Fraction);
        }
